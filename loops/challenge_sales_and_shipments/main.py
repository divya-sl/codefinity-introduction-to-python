...
for i in range(len(products)):
    for j in range(len(units_sold)):
        if(products[i][0] == units_sold[j][0]):
            products[i][1] -= units_sold[j][1]
            break

for i in range(len(products)):
    for j in range(len(shipment_received)):
        if(products[i][0] == shipment_received[j][0]):
            products[i][1] += shipment_received[j][1]
            break
